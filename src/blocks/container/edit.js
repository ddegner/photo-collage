...
import { PanelBody, ToggleControl, RangeControl, __experimentalUnitControl as UnitControl, Button } from '@wordpress/components';
import { useSelect, useDispatch } from '@wordpress/data';
import { createBlock } from '@wordpress/blocks';
import { __ } from '@wordpress/i18n';
import { SideBySideIcon, ThreeColumnsIcon, OverlapLeftIcon, OverlapRightIcon, ThreeGridIcon, OffsetGridIcon, ScatterIcon, HeroLayeredIcon, VerticalWaveIcon, StaggeredStoryIcon, OffsetGalleryIcon, CenterOverlayIcon } from './icons';
import './editor.scss';
...
const PRESETS = {
	...
	'scatter': [
		{ width: '40%', marginLeft: '10%', marginTop: '0%', rotation: -5, zIndex: 1 },
		{ width: '40%', marginLeft: '10%', marginTop: '-15%', rotation: 5, zIndex: 2 },
		{ width: '35%', marginLeft: '30%', marginTop: '-10%', rotation: 0, zIndex: 3 },
	],
	'hero-layered': [
...
const PRESET_BUTTONS = [
	...
	{ id: 'staggered-story', icon: StaggeredStoryIcon, label: 'Staggered Story' },
...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const applyPreset = (preset) => {
		...
		if (['vertical-wave', 'staggered-story', 'offset-gallery'].includes(preset)) {
			setAttributes({ containerHeight: '1200px' });
		} else if (['overlap-left', 'overlap-right', 'offset-grid', 'scatter', 'hero-layered', 'center-overlay'].includes(preset)) {
			setAttributes({ containerHeight: '' });
		} else if (['side-by-side', 'three-columns', 'three-grid'].includes(preset)) {
			setAttributes({ containerHeight: '' });
		}
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Quick Layouts', 'photo-collage')} initialOpen={true}>
					<div className="photo-collage-quick-layouts">
						{PRESET_BUTTONS.map((btn) => (
							...
							>
								{btn.icon}
								<span>{__(btn.label, 'photo-collage')}</span>
							</Button>
						))}
					</div>
				</PanelBody>
				<PanelBody title={__('Container Settings', 'photo-collage')}>
					<UnitControl
						label={__('Container Height', 'photo-collage')}
						value={containerHeight}
						onChange={(value) => setAttributes({ containerHeight: value })}
						help={__('Fixed height is required for absolute positioning and overlapping effects.', 'photo-collage')}
					/>
					<RangeControl
						...
						min={1}
						max={10}
						help={innerBlocks.length === 1
							? __('1 image in collage', 'photo-collage')
							: `${innerBlocks.length} ${__('images in collage', 'photo-collage')}`
						}
					/>
				</PanelBody>
				<PanelBody title={__('Responsive Settings', 'photo-collage')}>
					<ToggleControl
						label={__('Stack on Mobile', 'photo-collage')}
						help={__('Automatically stack images vertically on mobile devices.', 'photo-collage')}
